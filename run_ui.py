...
def requires_api_key(f):
    ...
    async def _async_impl(*args, **kwargs):
        res = _sync_impl(*args, **kwargs)
        if inspect.isawaitable(res):
            return await res
        return res

    return _async_impl if inspect.iscoroutinefunction(f) else _sync_impl
...
def _configure_wsgi_app():
    ...
    def register_api_handler(app, handler: type[ApiHandler]):
        ...
        # Ensure idempotent registration: skip if endpoint already exists
    ...
    # initialize and register API handlers (idempotent if re-imported)
...
def run():
    ...
    # Start init in the background so the server can accept connections immediately
...
# Expose ASGI app for uvicorn/hypercorn
try:
    # Wrap the WSGI app so ASGI servers (hypercorn/uvicorn) can run it
    asgi_app = WSGIMiddleware(_configure_wsgi_app())  # type: ignore
except Exception:
