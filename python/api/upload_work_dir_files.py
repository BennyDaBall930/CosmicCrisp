...
class UploadWorkDirFiles(ApiHandler):
    async def process(self, input: dict, request: Request) -> dict | Response:
        ...
        result = await runtime.call_development_function(
            get_work_dir_files.get_files,
            current_path if current_path != "$WORK_DIR" else ""
        )
...
async def upload_files(uploaded_files: list[FileStorage], current_path: str):
    if runtime.is_development():
        ...
        for file in uploaded_files:
            ...
            target_path = "" if current_path == "$WORK_DIR" else current_path
            if await runtime.call_development_function(
                upload_file, target_path, file.filename, base64_content
            ):
        ...
        browser = FileBrowser()
        target_path = "" if current_path == "$WORK_DIR" else current_path
        successful, failed = browser.save_files(uploaded_files, target_path)
