...
class BackupService:
    ...
    def get_default_backup_metadata(self) -> Dict[str, Any]:
        ...
        return {
            "backup_name": f"apple-zero-backup-{timestamp[:10]}",
    ...
    def _get_default_patterns(self) -> str:
        ...
        Only includes Apple Zero project directory patterns.
        """
...
{agent_root}/tmp/uploads/**"""
    ...
    async def _get_environment_info(self) -> Dict[str, Any]:
        ...
        try:
            return {
                ...
                "apple_zero_root": files.get_abs_path(""),
    ...
    def _translate_patterns(self, patterns: List[str], backup_metadata: Dict[str, Any]) -> List[str]:
        ...
        Replaces the backed up Apple Zero root path with the current Apple Zero root path
        ...
        """
        # Get the backed up apple zero root path from metadata
        environment_info = backup_metadata.get("environment_info", {})
        backed_up_agent_root = environment_info.get("apple_zero_root", "")

        # Get current apple zero root path
        ...
        for pattern in patterns:
            # Check if the pattern starts with the backed up apple zero root
            if pattern.startswith(backed_up_agent_root + '/') or pattern == backed_up_agent_root:
                ...
                # Pattern doesn't start with backed up apple zero root, keep as-is
    ...
    async def create_backup(
        ...
        backup_name: str = "apple-zero-backup"
    ) -> str:
        ...
        try:
            with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
                ...
                metadata = {
                    ...
                    "apple_zero_version": self.agent_zero_version,
                    "timestamp": datetime.datetime.now().isoformat(),
    ...
    def _translate_restore_path(self, archive_path: str, backup_metadata: Dict[str, Any]) -> str:
        ...
        # Check if the archive path starts with the backed up apple zero root
        if absolute_archive_path.startswith(backed_up_agent_root + '/') or absolute_archive_path == backed_up_agent_root:
            ...
            # Path doesn't start with backed up apple zero root, return as-is
