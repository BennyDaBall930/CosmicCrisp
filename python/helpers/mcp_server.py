...
mcp_server: FastMCP = FastMCP(
    name="Apple Zero integrated MCP Server",
    instructions="""
    Connect to remote Apple Zero instance.
    Apple Zero is a general AI assistant controlling it's macOS environment.
    Apple Zero can install software, manage files, execute commands, code, use internet, etc.
    Apple Zero's environment is isolated unless configured otherwise.
    """,
)
...
class ToolResponse(BaseModel):
    ...
    response: str = Field(
        description="The response from the remote Apple Zero Instance"
    )
...
class ToolError(BaseModel):
    ...
    error: str = Field(
        description="The error message from the remote Apple Zero Instance"
    )
    chat_id: str = Field(description="The id of the chat this message belongs to.")


SEND_MESSAGE_DESCRIPTION = """
Send a message to the remote Apple Zero Instance.
This tool is used to send a message to the remote Apple Zero Instance connected remotely via MCP.
"""
...
async def send_message(
    message: Annotated[
        str,
        Field(
            description="The message to send to the remote Apple Zero Instance",
            title="message",
        ),
    ],
    attachments: (
        Annotated[
            list[str],
            Field(
                description="Optional: A list of attachments (file paths or web urls) to send to the remote Apple Zero Instance with the message. Default: Empty list",
                title="attachments",
            ),
        ]
        | None
    ...
    Field(
        description="The response from the remote Apple Zero Instance", title="response"
    ),
]:
...
Finish a chat with the remote Apple Zero Instance.
This tool is used to finish a persistent chat (send_message with persistent_chat=True) with the remote Apple Zero Instance connected remotely via MCP.
If you want to continue the chat, use the send_message tool instead.
Always use this tool to finish persistent chat conversations with remote Apple Zero.
"""
...
class DynamicMcpProxy:
    ...
    def reconfigure(self, token: str):
        ...
        # Create new MCP apps with updated settings (FastMCP >= 2.10 API)
        with self._lock:
            # Build SSE app without auth by default; auth can be layered via middleware later.
    ...
    def _create_custom_http_app(self, streamable_http_path, auth_server_provider, auth_settings, debug, routes):
        ...
        # Add StreamableHTTP routes (no-auth by default)
