...
class MemoryConsolidator:
    ...
    async def _process_memory_with_consolidation(
        ...
        if similar_memories:
            ...
            db = await Memory.get(self.agent)
            still_existing = await db.get_by_ids(memory_ids_to_check)
    ...
    async def _gather_consolidated_metadata(
        ...
        try:
            ...
            if memory_ids:
                original_memories = await db.get_by_ids(memory_ids)
    ...
    async def _handle_replace(
        ...
        if result.memories_to_remove:
            ...
            memories_to_check = await db.get_by_ids(result.memories_to_remove)
    ...
    async def _handle_update(
        ...
        for update_info in result.memories_to_update:
            ...
            if memory_id and new_content:
                ...
                existing_docs = await db.get_by_ids([memory_id])
