...
class SchedulerTaskList(BaseModel):
    ...
    def get(cls) -> "SchedulerTaskList":
        ...
        if cls.__instance is None:
            if not exists(path):
                make_dirs(path)
                # Initialize and save using the shared background event loop
                elt = EventLoopThread("Background")
                inst = cls(tasks=[])
                elt.run_coroutine(inst.save()).result()
                cls.__instance = inst
            else:
                cls.__instance = cls.model_validate_json(read_file(path))
        else:
            # Refresh from disk without creating a new event loop
            elt = EventLoopThread("Background")
            elt.run_coroutine(cls.__instance.reload()).result()
