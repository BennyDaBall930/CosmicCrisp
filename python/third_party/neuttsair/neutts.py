...
import re
import perth
# Don't import neucodec at top level - import on-demand in _load_codec
# This avoids import failures cascading to the bundled stubs
NeuCodec = None  # type: ignore
DistillNeuCodec = None  # type: ignore
NeuCodecOnnxDecoder = None  # type: ignore
...
class NeuTTSAir:
    ...
    def __init__(
        ...
        # Encoder (may be separate from decoder for ONNX)
        self.encoder = None
        ...
        # Load watermarker (fallback to no-op if perth is unavailable)
        watermarker_cls = getattr(perth, "PerthImplicitWatermarker", None)
        if callable(watermarker_cls):
            self.watermarker = watermarker_cls()
        else:
            print("Warning: PerthImplicitWatermarker unavailable; voice output will be unwatermarked.")

            class _NoopWatermarker:
                def apply_watermark(self, audio, sample_rate=24_000):
                    return audio
    ...
    def _load_codec(self, codec_repo, codec_device):
        ...
        match codec_repo:
            case "neuphonic/neucodec":
                # Import full neucodec dynamically
                try:
                    import importlib
                    neucodec_pkg = importlib.import_module('neucodec')
                    if not hasattr(neucodec_pkg, 'NeuCodec'):
                        raise ImportError("neucodec package missing NeuCodec class")
                    _NeuCodec = neucodec_pkg.NeuCodec
                except Exception as e:
                    raise ImportError(f"Failed to import full neucodec: {e}") from e
                
                self.codec = _NeuCodec.from_pretrained(codec_repo)
                self.codec.eval().to(codec_device)
                self.encoder = self.codec  # Use same codec for encoding
            case "neuphonic/distill-neucodec":
                # Import full neucodec dynamically
                try:
                    import importlib
                    neucodec_pkg = importlib.import_module('neucodec')
                    if not hasattr(neucodec_pkg, 'DistillNeuCodec'):
                        raise ImportError("neucodec package missing DistillNeuCodec class")
                    _DistillNeuCodec = neucodec_pkg.DistillNeuCodec
                except Exception as e:
                    raise ImportError(f"Failed to import full neucodec: {e}") from e
                
                self.codec = _DistillNeuCodec.from_pretrained(codec_repo)
                self.codec.eval().to(codec_device)
                self.encoder = self.codec  # Use same codec for encoding
            case "neuphonic/neucodec-onnx-decoder":
                ...
                try:
                    from neucodec import NeuCodecOnnxDecoder as _HubNeuCodecOnnxDecoder
                except ImportError:
                    from ._onnx_decoder import NeuCodecOnnxDecoder as _HubNeuCodecOnnxDecoder

                # Load ONNX decoder for fast decoding. Encoding is not supported in this mode.
                self.codec = _HubNeuCodecOnnxDecoder.from_pretrained(codec_repo)
    ...
    def encode_reference(self, ref_audio_path: str | Path):
        """Encode reference audio to codes using the encoder (not ONNX decoder)."""
        if self._is_onnx_codec:
            raise NotImplementedError(
                "Reference encoding is unavailable when using the ONNX decoder. "
                "Use pre-encoded reference files instead."
            )
        if self.encoder is None:
            raise RuntimeError(
                "Encoder not available. Cannot encode reference audio. "
                "Ensure the full neucodec package is installed."
            )
