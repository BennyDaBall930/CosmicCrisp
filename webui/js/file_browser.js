...
openFileLink = async function (path) {
  try {
    ...
    if (resp.is_dir) {
      if (resp.in_base) {
        // Open inside the HoneyCrisp project root
        fileBrowserModalProxy.openModal(resp.browser_path || "$WORK_DIR");
      } else {
        window.toastFrontendError("Only project files can be browsed.", "File Browser");
        fileBrowserModalProxy.openModal("$WORK_DIR");
      }
    } else {
      if (!resp.in_base) {
        window.toastFrontendError("Only project files can be downloaded.", "File Download");
        return;
      }
      fileBrowserModalProxy.downloadFile({
        path: resp.browser_path,
        name: resp.file_name,
      });
    }
  } catch (e) {
