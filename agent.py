...
class Agent:
    ...
    def __init__(
        ...
        # Initialize extensions using the shared background event loop.
        # Avoid deadlock when called from the same loop thread by not blocking.
        try:
            loop_thread = EventLoopThread("Background")
            fut = loop_thread.run_coroutine(self.call_extensions("agent_init"))
            # Only block if not on the same event loop thread
            if threading.current_thread().name != loop_thread.thread_name:
                fut.result()
            # else: we're already on that loop thread; do not block
        except Exception:
            # Non-fatal: allow agent construction even if init extensions fail
            pass
